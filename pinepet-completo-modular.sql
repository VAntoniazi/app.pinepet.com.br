...
DO $verify$
DECLARE
 required text[]:=ARRAY['acesso_usuarios','organizacao_estabelecimentos','onboarding_processos','clientes','clientes_sexos','pets','pets_especies','pets_racas','pets_portes','equipe_profissionais','organizacao_configuracoes','saude_responsaveis_tecnicos','fiscal_configuracoes','financeiro_metodos_pagamento','financeiro_estabelecimentos_metodos','organizacao_horarios','atendimento_fluxos','atendimento_fluxos_preferencias','sistema_usuarios_permissoes','onboarding_importacoes','onboarding_importacao_lotes'];
 item text;missing text[]:=ARRAY[]::text[];
BEGIN
 FOREACH item IN ARRAY required LOOP IF to_regclass('pinepet.'||item)IS NULL THEN missing:=array_append(missing,item);END IF;END LOOP;
 IF to_regprocedure('pinepet.fn_cpf_valido(text)')IS NULL THEN missing:=array_append(missing,'fn_cpf_valido(text)');END IF;
 IF to_regprocedure('pinepet.fn_cnpj_valido(text)')IS NULL THEN missing:=array_append(missing,'fn_cnpj_valido(text)');END IF;
 IF cardinality(missing)>0 THEN RAISE EXCEPTION 'Estrutura global incompleta: %',array_to_string(missing,', ');END IF;
 RAISE NOTICE 'Estrutura global PinePet validada com sucesso até a migration 010.';
END$verify$;

SELECT version,description,applied_at FROM sistema_schema_versions ORDER BY applied_at,version;
SELECT tabela,registro_id,documento,motivo,ocorrido_em FROM sistema_documentos_saneamento ORDER BY id;
