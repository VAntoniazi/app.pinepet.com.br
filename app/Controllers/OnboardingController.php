...
final class OnboardingController
{
    ...
    public function store(Request$request):never
    {
        ...
        $payload=null;try{if($step===6){$migrate=$request->input('migration_choice')==='1';$ids=json_decode((string)$request->input('completed_imports','[]'),true,16,JSON_THROW_ON_ERROR);if($migrate)$this->imports->assertCompleted(is_array($ids)?$ids:[],(int)$user['user_id'],(int)$user['business_id']);$name=$this->onboarding->complete((int)$user['user_id'],(int)$user['business_id']);Csrf::rotate();Auth::completeProfile($name);flash('success','Configuracao concluida. Bem-vindo ao PinePet!');Response::redirect('/painel');}$payload=match($step){1=>$this->identity($request),2=>$this->vaccines($request),3=>$this->fiscal($request,(int)$user['business_id']),4=>$this->payments($request),5=>$this->hours($request),default=>throw new \RuntimeException('Etapa invalida.')};$key=[1=>'identidade',2=>'vacinas',3=>'fiscal',4=>'pagamentos',5=>'horarios'][$step];$this->onboarding->save((int)$user['user_id'],(int)$user['business_id'],$step,$key,$payload);}
    ...
    {
        $name=$this->clean($r->input('name'),120);$birth=trim((string)$r->input('birth_date'));$sex=trim((string)$r->input('sex'));$cpf=$this->digits($r->input('cpf'));$has=$r->input('has_cnpj')==='1';$cnpj=$this->cnpjValue($r->input('cnpj'));$trade=$this->clean($r->input('trade_name'),120);$role=$this->clean($r->input('role'),40);$specialty=$this->clean($r->input('specialty'),180);
        $date=\DateTimeImmutable::createFromFormat('!Y-m-d',$birth);if(mb_strlen($name)<3||!str_contains($name,' '))throw new \RuntimeException('Informe seu nome completo.');if(!$date||$date->format('Y-m-d')!==$birth||$date>new \DateTimeImmutable('-18 years'))throw new \RuntimeException('Informe uma data de nascimento valida.');if(!in_array($sex,['feminino','masculino','intersexo','nao_informar'],true)||!$this->document($cpf,11))throw new \RuntimeException('Revise seus dados pessoais.');if(!in_array($role,['Proprietario','Socio','Gestor','Profissional'],true))throw new \RuntimeException('Selecione sua funcao no estabelecimento.');
        $company=[];if($has){if(!$this->validCnpj($cnpj))throw new \RuntimeException('Informe um CNPJ valido.');$this->throttleCnpj();$company=$this->cnpj->lookup($cnpj);if(($company['razao_social']??null)===null)throw new \RuntimeException('A consulta nao retornou a razao social do estabelecimento.');}elseif(mb_strlen($trade)<2)throw new \RuntimeException('Informe o nome do estabelecimento.');
        return['name'=>$name,'birth_date'=>$birth,'sex'=>$sex,'cpf'=>$cpf,'has_cnpj'=>$has,'cnpj'=>$cnpj,'trade_name'=>$trade,'role'=>$role,'specialty'=>$specialty,'company'=>$company];
    }
    private function vaccines(Request$r):array
    {
        $yes=$r->input('aplica_vacinas')==='1';$data=['aplica_vacinas'=>$yes];if(!$yes)return$data;$data+=['rt_name'=>$this->clean($r->input('rt_name'),240),'rt_cpf'=>$this->digits($r->input('rt_cpf')),'rt_council'=>strtoupper($this->clean($r->input('rt_council'),20)),'rt_number'=>$this->clean($r->input('rt_number'),40),'rt_uf'=>strtoupper($this->clean($r->input('rt_uf'),2)),'rt_validity'=>trim((string)$r->input('rt_validity')),'rt_email'=>mb_strtolower($this->clean($r->input('rt_email'),254)),'rt_phone'=>$this->digits($r->input('rt_phone'))];if(mb_strlen($data['rt_name'])<3||!$this->document($data['rt_cpf'],11)||!in_array($data['rt_council'],['CRMV'],true)||preg_match('/^[A-Z]{2}$/',$data['rt_uf'])!==1||$data['rt_number']==='')throw new \RuntimeException('Preencha os dados validos do responsavel tecnico.');if($data['rt_email']!==''&&!filter_var($data['rt_email'],FILTER_VALIDATE_EMAIL))throw new \RuntimeException('Informe um e-mail valido para o responsavel tecnico.');return$data;
    }
