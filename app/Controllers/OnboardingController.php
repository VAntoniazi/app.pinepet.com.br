...
final class OnboardingController
{
    ...
    public function store(Request$request):never
    {
        ...
        catch(\Throwable$e){if(is_array($payload)&&isset($payload['certificado']['path'])){$path=BASE_PATH.'/'.ltrim((string)$payload['certificado']['path'],'/');if(str_starts_with($path,BASE_PATH.'/storage/private/certificates/')&&is_file($path))unlink($path);}error_log('Onboarding step '.$step.': '.$e->getMessage());$safe=$e instanceof \RuntimeException&&!$e instanceof \PDOException?$e->getMessage():($step===6?'Não foi possível concluir a configuração. Tente novamente.':'Não foi possível salvar esta etapa.');$old=$_POST;unset($old['_token'],$old['certificate_password']);flash('error',$safe);flash('old',$old);Response::redirect('/primeiro-acesso');}Response::redirect('/primeiro-acesso');
    }
    private function identity(Request$r,int$userId):array
    {
        $profile=$this->profiles->data($userId);if(!$profile)throw new \RuntimeException('Nao foi possivel recuperar os dados do cadastro.');$name=(string)$profile['nome_completo'];$birth=(string)$profile['data_nascimento'];$sex=(string)$profile['sexo_biologico'];$cpf=$this->digits($profile['cpf']);$has=$r->input('has_cnpj')==='1';$cnpj=$this->cnpjValue($r->input('cnpj'));$trade=$this->clean($r->input('trade_name'),120);$role=$this->clean($r->input('role'),40);
        if(!$this->document($cpf,11))throw new \RuntimeException('O CPF do cadastro inicial é inválido. Corrija o cadastro antes de continuar.');if(!in_array($role,['Proprietario','Socio','Administrador','Gerente','Veterinario','Recepcionista','Groomer','Auxiliar veterinario','Financeiro','Estoque','Outra funcao'],true))throw new \RuntimeException('Selecione sua funcao no estabelecimento.');
