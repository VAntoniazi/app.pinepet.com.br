...
final class AuthController
{
    ...
    public function login(Request $request): never
    {
        ...
        if(password_needs_rehash($user['senha'],PASSWORD_DEFAULT)) $this->users->updatePasswordHash((int)$user['id'],password_hash($password,PASSWORD_DEFAULT));
        Csrf::rotate(); Auth::login($user); Response::redirect(empty($user['cadastro_completo_em'])?'/primeiro-acesso':'/painel');
    }
    public function logout(Request $request): never { if(!Csrf::validate($request->input('_token'))) Response::abort(419); Auth::logout(); Response::redirect('/entrar'); }
    ...
    public function activate(Request $request): never
    {
        ...
        Csrf::rotate(); Auth::login($user); Response::redirect('/primeiro-acesso');
    }
}
