...
final class AuthRepository
{
    ...
    {
        $s=$this->database->pdo()->prepare('SELECT id,public_id,id_estabelecimento,nome_completo,email,senha,status,cadastro_completo_em FROM cadastro_usuarios WHERE email=:email AND deleted_at IS NULL LIMIT 1');
    ...
    {
        $this->database->pdo()->prepare('UPDATE cadastro_usuarios SET senha=:password,updated_at=NOW(),version_lock=version_lock+1 WHERE id=:id AND deleted_at IS NULL')->execute(['password'=>$passwordHash,'id'=>$userId]);
    }
    public function activate(string $tokenHash,string $passwordHash): ?array
    {
        return $this->database->transaction(function(PDO $pdo) use($tokenHash,$passwordHash): ?array {
            $s=$pdo->prepare("SELECT t.id AS draft_id,u.id,u.public_id,u.id_estabelecimento,u.nome_completo,u.email,u.cadastro_completo_em FROM cadastro_temporarios t JOIN cadastro_usuarios u ON u.id=t.id_usuario WHERE t.ativacao_token_hash=:hash AND t.ativacao_expira_em>NOW() AND u.deleted_at IS NULL LIMIT 1 FOR UPDATE OF t,u");
