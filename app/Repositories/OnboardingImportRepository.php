...
final class OnboardingImportRepository
{
    ...
    public function processBatch(string$id,int$userId,int$businessId,int$batch,string$key,string$payloadHash,array$rows,callable$importer):array
    {
        return $this->database->transaction(function(PDO$pdo)use($id,$userId,$businessId,$batch,$key,$payloadHash,$rows,$importer):array{
            $job=$this->job($id,$userId,$businessId,true);if(!$job)throw new \RuntimeException('Importação não encontrada.');
            $existing=$pdo->prepare('SELECT payload_hash,resultado FROM onboarding_importacao_lotes WHERE id_importacao=:id AND numero_lote=:batch');$existing->execute(['id'=>$id,'batch'=>$batch]);$done=$existing->fetch();if(is_array($done)){if(!hash_equals((string)$done['payload_hash'],$payloadHash))throw new \RuntimeException('O arquivo foi alterado depois de uma importação já processada. Troque o arquivo ou restaure os dados anteriores.');return json_decode((string)$done['resultado'],true,16,JSON_THROW_ON_ERROR);}if(!in_array($job['status'],['PREPARADA','PROCESSANDO'],true))throw new \RuntimeException('Esta importação já foi concluída.');
