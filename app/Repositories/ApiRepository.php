...
final class ApiRepository
{
    ...
    public function clients(int$id,int$limit,int$offset):array{return$this->many('SELECT id,nome,sobrenome,email,whatsapp_com_ddd,criado_em,atualizado_em FROM clientes WHERE id_estabelecimento=:business ORDER BY id DESC',$id,$limit,$offset);}
    public function pets(int$id,int$limit,int$offset):array{return$this->many("SELECT p.id,p.nome,p.data_nascimento,p.criado_em,p.atualizado_em,COALESCE((SELECT jsonb_agg(jsonb_build_object('id_cliente',c.id,'nome',c.nome,'sobrenome',c.sobrenome) ORDER BY c.nome) FROM pets_tutores pt JOIN clientes c ON c.id=pt.id_cliente AND c.id_estabelecimento=pt.id_estabelecimento WHERE pt.id_pet=p.id AND pt.id_estabelecimento=p.id_estabelecimento),'[]'::jsonb) AS tutores FROM pets p WHERE p.id_estabelecimento=:business ORDER BY p.id DESC",$id,$limit,$offset);}
